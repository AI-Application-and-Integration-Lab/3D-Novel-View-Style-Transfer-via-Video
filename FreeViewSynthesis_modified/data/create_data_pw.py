...
def run(dense_dir, scale, dm_write_vis=True):
    ...
    im_paths = []
    # im_paths += sorted((dense_dir / "images").glob("*.png")) # colmap
    # im_paths += sorted((dense_dir / "images").glob("*.jpg"))
    # im_paths += sorted((dense_dir / "images").glob("*.jpeg"))
    im_paths += sorted((pw_dir).glob("im_*.png")) # own data
    im_paths += sorted((pw_dir).glob("im_*.jpg")) 
    im_paths += sorted((pw_dir).glob("im_*.jpeg")) 
    ...
    # Ks, Rs, ts = co.colmap.load_cameras(dense_dir / "sparse", im_paths, scale) # colmap

    # print(f"write camera params to")
    # np.save(pw_dir / "Ks.npy", Ks)
    # np.save(pw_dir / "Rs.npy", Rs)
    # np.save(pw_dir / "ts.npy", ts)
    Ks = np.load(pw_dir / "Ks.npy") # own data
    Rs = np.load(pw_dir / "Rs.npy")
    ts = np.load(pw_dir / "ts.npy")

    # mesh_path = dense_dir / "meshed-delaunay.ply" #colmap
    # dm_paths = render_depth_maps_mesh(
    #     pw_dir, mesh_path, Ks, Rs, ts, height, width, write_vis=dm_write_vis
    # )
    dm_paths = sorted(pw_dir.glob("dm_*.npy")) # own data
