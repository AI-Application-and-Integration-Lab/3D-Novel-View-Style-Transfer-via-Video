...
def run(dense_dir, scale, dm_write_vis=True):
    ...
    im_paths = sorted(pw_dir.glob(f"im_*.png"))
    im_paths += sorted(pw_dir.glob(f"im_*.jpg"))
    im_paths += sorted(pw_dir.glob(f"im_*.jpeg"))
    ...
    # )
    dm_paths = sorted(pw_dir.glob("dm_*.npy"))
