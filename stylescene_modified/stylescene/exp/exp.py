...
class Worker(co.mytorch.Worker):
  ...
  def copy_data(self, data, device, train):
    ...
    for k, v in data.items():
      if isinstance(v, list):
        self.data[k] = [item.to(device).requires_grad_(requires_grad=False) for item in v]
      else:
        self.data[k] = v.to(device).requires_grad_(requires_grad=False)

  def net_set_empty_state(self, net):
        net.matrix.prev_hidden = None
  ...
  def callback_eval_add(self, **kwargs):
    ...
    if not config.Train:
      ...
      es_wrp = self.im_to2np(output["out_wrp"]) if output["out_wrp"] is not None else None
      timestamp = output["timestamp"].item()
      ...
      for b in range(ta.shape[0]):
        ...
        for c in range(out_im.shape[0]):
          ...
          # out.save(out_dir / f"{c:02d}_s{bidx:04d}_es_l.jpg")
          out.save(out_dir / f"{config.Test_style[c]:03d}_s{bidx:04d}_es.jpg")
        if es_wrp is not None:
          out_im_wrp = (255 * es_wrp[b]).astype(np.uint8)
          for c in range(out_im_wrp.shape[0]):
            out = PIL.Image.fromarray(out_im_wrp[c])
            out.save(out_dir / f"{config.Test_style[c]:03d}_s{bidx-timestamp:04d}_es_r.jpg")
...
if __name__ == "__main__":
  ...
  parser.add_argument("--eval-style-id", type=int, default=2)
  args = parser.parse_args()

  if not config.Train:
    config.Test_style = [args.eval_style_id]
