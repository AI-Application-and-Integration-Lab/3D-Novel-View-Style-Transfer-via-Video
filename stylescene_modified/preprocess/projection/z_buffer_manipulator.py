...
class PtsManipulator(nn.Module):
    ...
    def project_pts_cumulative(
        self, pts3D, K, K_inv, RT_cam, RTinv_cam, prior_point_cloud=None, last_background_mask=None
    ):
        # PERFORM PROJECTION
        # Project the world points into the new view
        if last_background_mask is not None:
    ...
    def project_pts_cumulative_yc(
        ...
        if prior_point_cloud is not None:
            xy_proj2 = prior_point_cloud
            xy_proj = torch.cat([xy_proj2, xy_proj1], axis=2)
        else:
            xy_proj = xy_proj1
