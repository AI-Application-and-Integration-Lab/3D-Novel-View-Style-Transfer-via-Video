...
def run():
    ...
    mstyler.load_state_dict(torch.load('weights/model_iter_14000.pth.tar')['state_dict'], strict=True)
    mstyler.train()
    vgg = vgg16.Vgg16()
    vgg.eval()
    ...
    for i in range(800000):
        ...
        frame_o_R = []
        for t in range(10):
            frame_i_L.append(contentsL[:, t, :, :, :])
            frame_i_R.append(contentsR[:, t, :, :, :])
        style = Variable(style.cuda(), requires_grad=False)
        if style.shape[0] != contentsL.shape[0]:
            style = style[:contentsL.shape[0]]

        loss = 0
        for t1 in range(9):
            loss_c, loss_s, loss_t, outL, outR, prev_state1, prev_state2 = network.forward_LR_video(frame_i_L[t1].to(device), frame_i_L[t1+1].to(device), frame_i_R[t1].to(device), style.to(device), prev_state1, prev_state2, bank)
            prev_state1 = repackage_hidden(prev_state1)
            prev_state2 = repackage_hidden(prev_state2)

            frame_o_L.append(outL.detach())
            frame_o_R.append(outR.detach())

            if t1 > 0 :
                loss_t += network.temporal_loss(frame_i_R[t1-1].to(device), frame_i_R[t1].to(device), frame_o_R[t1-1].to(device), outR.to(device))
            ...
            for t2 in range(1, t1-4):
                loss_t = network.temporal_loss(frame_i_L[t2].to(device), frame_i_L[t1+1].to(device), frame_o_L[t2-1].to(device), outL.to(device))
                loss_t = loss_t * args.long_weight / (t1-5)
                loss += loss_t
                loss_t = network.temporal_loss(frame_i_R[t2-1].to(device), frame_i_R[t1].to(device), frame_o_R[t2-1].to(device), outR.to(device))
                loss_t = loss_t * args.long_weight / (t1-5)
                loss += loss_t
            optimizer.zero_grad()
            loss.backward()
        ...
        if (i + 1) % 100 == 0:
            for j in range(5):
                out = frame_o_L[j][0].detach().cpu().numpy().transpose(1,2,0)
                out = Image.fromarray((out*255).astype(np.uint8))
                out.save(f'stylize_log/{i:06}_{j}_L.png')
                out = frame_o_R[j+1][0].detach().cpu().numpy().transpose(1,2,0)
                out = Image.fromarray((out*255).astype(np.uint8))
                out.save(f'stylize_log/{i:06}_{j}_R.png')
