...
from models.stylize import net, styler2, vgg16
from flow import *
...
if __name__ == '__main__':
    ...
    mstyler.eval()
    model_checkpoint = torch.load('weights/120style.pth.tar')
    ...
    for bank in range(20):
        ...
        for i in range(len(content_loader)):
            ...
            cur_content_L, _, _, _ = data
            cur_content_L = cur_content_L.cuda()
        
            cur_content_L = vgg_norm(cur_content_L)
            with torch.no_grad():
                out, prev_state1, prev_state2 = mstyler(cur_content_L, prev_state1, prev_state2, bank)
        
            prev_state1 = repackage_hidden(prev_state1)
            prev_state2 = repackage_hidden(prev_state2)
            res0 = out[0].detach().cpu().numpy().transpose(1,2,0)
